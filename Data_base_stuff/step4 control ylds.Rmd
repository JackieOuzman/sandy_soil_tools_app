...
summary_control_data_all <- summary_control_data_all %>% 
  mutate(soil_modification =  str_extract(summary_control_data_all$Descriptors, "[^.]+"))#keep everything before the first 
head(summary_control_data_all)
#the Unmodified seems to have a problem this fixes it :)
summary_control_data_all <- summary_control_data_all %>% 
  mutate(soil_modification = str_extract(summary_control_data_all$soil_modification, "[^_]+")) #keep everything after the first _
...
soil_modifications <- summary_control_data_all %>% 
  distinct(soil_modification) %>% 
  arrange(soil_modification) %>% 
  filter(!is.na(soil_modification)) %>% 
  filter(soil_modification != "none" ) 

count(soil_modifications)
...
amendment_with_depth <- summary_control_data_all %>% 
  distinct(amendment_all) %>% 
  arrange(amendment_all) %>% 
  filter(!is.na(amendment_all)) %>% 
...
cum_yld_gains_control <- cum_yld_gains_control %>% 
  mutate(soil_modification =  str_extract(cum_yld_gains_control$Descriptors, "[^.]+"))#keep everything before the first 
head(cum_yld_gains_control)
#the Unmodified seems to have a problem this fixes it :)
cum_yld_gains_control <- cum_yld_gains_control %>% 
  mutate(soil_modification = str_extract(cum_yld_gains_control$soil_modification, "[^_]+")) #keep everything after the first _
