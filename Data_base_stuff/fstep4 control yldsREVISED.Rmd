...
How many sites, descriptors, soil modification (including different depths)?
soil amendment (including different depths)
How many soil modification (including different depths)?

JAXS - make this into a table
...
cum_yld_control <- summary_control_data_all %>% 
  dplyr::group_by(site, Descriptors,rep_block, crop, soil_modification, amendment_123, amendment_code1 ) %>% 
...
The below graph shows that doing 'anything' is likely to lift you yield compaired to a control
...
- Is this saying that sweep only gives yield gains when used with amendments?
...
- The length of the trial does not always reflect higher yield
  for example trials that ran for 6 years had low cumulative yields
...
JAX 
 ...
 - Heaps of zero values for treatment yield 
 - This should be checked Waik, Tempy, Cadgee and Buckleboo
 - higher values for treatment yields are at Brimpton lake and Bute Trengrove (these look OK)
 
 - Heaps of zero values for control yield
 - The higher control yield tend to be lower than the treatment yield - which is good check
 - the high control yields occur at Karkoo and Bute CSIRO
...
- JAX what would this be? should it mostly be the control?

- might be on row sowing ??
