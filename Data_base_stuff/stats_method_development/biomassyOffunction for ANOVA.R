...
#  max(sites_names)
 ...
 ## test 
 site_yrs_list <- c( "TempyX2021") #no yield
...
summary_data_site$dry_biomass<- as.double(summary_data_site$dry_biomass)

## count yld values
count_missing_yld_values <- summary_data_site %>% 
  dplyr::group_by(Descriptors) %>% 
  dplyr::summarise(count = sum(!is.na(dry_biomass)))
...
data_summary_all_analysis <- summary_data_site %>% 
  dplyr::group_by(Descriptors) %>%
  dplyr::summarise(mean=mean(dry_biomass, na.rm = TRUE), 
                   sd=sd(dry_biomass, na.rm = TRUE),
                   #count = n(),
...
model = lm( dry_biomass ~ Descriptors,
...
bartlett.test <- bartlett.test(dry_biomass ~ Descriptors,
              data = summary_data_site) 
...
########################################################################################################################################################
#### agricolae_LSD
model_sand = lm( dry_biomass ~ Descriptors,
                 data=summary_data_site)
...
DunnettTest <- DunnettTest(dry_biomass ~ Descriptors,
                           data = summary_data_site,
                           control = "Control")
...
write.csv(Tempy_2021_ANOVA,"X:/Therese_Jackie/Sandy_soils/Development_database/stats_batch_output/Tempy_2021_ANOVA.csv" ,
          row.names = FALSE)
