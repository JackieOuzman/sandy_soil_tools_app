...
*Step 1a* I have fitted a linear model to my data.

*Step 1b* One-way ANOVA using type II, or I or III.

*Step 1c* which is the best fit of model (can be used if choosing between one-way or two-way ANOVA)
...
Step 1 is to descide what type of ANOVA you need.
...
one.way <- aov(variable 1  ~ varaiable 2, data = df)
...
If the p-value of the 'varaible 2' variable is low (p < 0.001), then it appears that the type of 'varaible 2' has a real impact on the "variable 1".


### Step 1a and 1b Fit the model and conduct One-way ANOVA 

####In the *example dataset* I fit a linear model
 
```{r fit model example, message=FALSE, warning=FALSE, include=TRUE}
...
#### Fit the linear model and conduct One-way ANOVA for *sandy soil dataset*
 
```{r fit model sandys, message=FALSE, warning=FALSE, include=TRUE}
...
When the dataset is simple and balanced the Type I , II and III should give you very similar results.

In sand and the example data they all come out as significant, type I and II are the same.

Type III has a smaller P value suggesting that the test is more likley to report differences.
...
#### *Example dataset* results for type I II III ANOVA.
...
p_value_eg_anovaI <- anova_I[1,4]
p_value_eg_anovaII <- anova_II[1,4]
p_value_eg_anovaIII <- anova_III[1,4]
...
p_value_comp_table_example <- data.frame(Type_of_ANOVA, P_values)
...
#### *Sands dataset* results for type I II III ANOVA.


```{r ANOVA type sandys, echo=FALSE, message=FALSE, warning=FALSE}
model_sand = lm( yield ~ Descriptors,
                 data=summary_data_site)
...
summary(mc_sand_control, test=adjusted("single-step")) # I dont think I want this I think this uses dependencies
summary(mc_sand_control, test=adjusted("none")) #perhaps this is better - I need to find out more
...
### Multiple comparisons least square means  with example dataset
...
### Multiple comparisons least square means  with sand dataset
...
### Graphing the results example data
...
### Graphing the results sand data

Simple box plots of values across groups
should be able to do something better with ggplot
 
```{r Graphing the results sand , include=TRUE} 

boxplot(yield ~ Descriptors,
        data = summary_data_site,
        ylab="yield",
        xlab="Descriptors")
```
...
### Plot using ggplot example data
...
Example of presenting results using Fishers (from one of Therses papers)

Analysis of Variance (ANOVA) was undertaken using
the GENSTAT version 13 statistical package (VSN
International, Rothamsted, UK). 

Assumptions of constant error variance (homogeneity), normality of data
distribution and additivity of treatment and replicate
effects were tested for each analysis. 

Least significant difference (l.s.d.) between treatments was determined at
<5 % significance using Fisher’s protected l.s.d.
