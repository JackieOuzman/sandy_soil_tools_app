...
site_yrs_list <- c(
                   ...
                   "WynarkaX2019to2021",
                   "YendaX2017to2021")#,
                  # "YounghusbandX2020to2021" run in different script
                   #)
...
cumulative_yld_table <- cumulative_yld_table %>%
  mutate(ANOVA_sign_0.1 = case_when(
    ...
    p_value_ANOVA_cum <= 0.1 ~  "*",
    p_value_ANOVA_cum >  0.1 ~  "ns",
    TRUE ~ "check"
...
tukey_agricolae_cum <- (HSD.test(model_cum_LSD, "Descriptors", alpha = 0.1))
...
Cum_ANOVA_sites_yr <- rbind(
  ...
  Lowaldie_Crest_2019to2021_Cum_ANOVA,
  `Lowaldie_Deep sand_2019to2021_Cum_ANOVA`,
  ...
  #Younghusband_2020to2021_Cum_ANOVA #run in different script
...
write.csv(Cum_ANOVA_sites_yr,"X:/Therese_Jackie/Sandy_soils/Development_database/stats_batch_output/final_method/Cum_ANOVA_sites_merged_90_no_Younghusband.csv" ,
                             row.names = FALSE)
