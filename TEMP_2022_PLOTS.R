...
site_yrs_list = "BrookerX2019to2021" # set the site name this will be a filter with R shiny 'input$site_selection'
site_selection_in_app <- "Brooker"

### name of site selected

a <-  site_selection_in_app #name of the site
b <-  trial_results %>% 
  filter(site == a) %>% 
  distinct(year)
...
################                 ANOVA Results                    ################
###############################################################################################################################################
# bring in the ANOVA results 


#Cum_ANOVA_results <- read.csv("Data_base_stuff/stats/ # I need to fix this so it Git Hub tracks this file and then I can use relative paths
Cum_ANOVA_results <- read.csv("X:/Therese_Jackie/Sandy_soils/Development_database/stats_batch_output/final_method/Cum_ANOVA_sites_merged_90.csv")
Cum_ANOVA_results$Descriptors <- factor(Cum_ANOVA_results$Descriptors,
                                        levels = order)


#filter the ANOVA data on site selected
...
site_year_yld_summary_site <- yld_table %>%
  filter(site == a) 
